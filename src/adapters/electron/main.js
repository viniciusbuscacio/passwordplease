...
ipcMain.handle('vault:lock', async () => { clearLockTimer(); if (controller) await controller.lock(); currentDbPath = null; mainWindow?.setTitle('passwordPlease'); if (tray && fs.existsSync(LOCKED_ICON)) tray.setImage(LOCKED_ICON); return { ok: true }; });
